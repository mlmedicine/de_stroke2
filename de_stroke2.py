...
HCY = col2.number_input('HCY (μmol/L)',value=17.7,step=0.1,format="%.1f")
CRP = col3.number_input('CRP (mg/L)',step=0.1,format="%.1f",value=14.6)
...
# RF.fit(X_ros, y_ros)
XGB = XGBClassifier(random_state=12,max_depth=3,n_estimators=51)
XGB.fit(X_ros, y_ros)
#读之前存储的模型
...
sp = 0.5
#figure
is_t = (XGB.predict_proba(np.array([[ SOH, HCY,CRP, SS]]))[0][1])> sp
prob = (XGB.predict_proba(np.array([[SOH, HCY,CRP, SS]]))[0][1])*1000//1/10
